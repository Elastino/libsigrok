...
static int zip_create(const struct sr_output *o)
{
	...
	if (zip_file_add(zipfile, "version", versrc, ZIP_FL_ENC_UTF_8) < 0) {
	...
	if (zip_file_add(zipfile, "metadata", metasrc, ZIP_FL_ENC_UTF_8) < 0) {
	...
	uint8_t *buf, size_t unitsize, size_t length)
{
	...
	if (!g_key_file_has_key(kf, "device 1", "unitsize", &error)) {
		...
		if (zip_file_replace(archive, zs.index, metasrc, ZIP_FL_ENC_UTF_8) < 0) {
	...
	for (i = 0; i < num_files; i++) {
		...
		if (entry_name[7] == '\0') {
			 ...
			 * with chunk 2.
			 */
			if (zip_file_rename(archive, i, "logic-1-1", ZIP_FL_ENC_UTF_8) < 0) {
	...
	const float *values, size_t count, size_t ch_nr)
{
	...
	i = zip_file_add(archive, chunkname, analogsrc, ZIP_FL_ENC_UTF_8);
	if (i < 0) {
